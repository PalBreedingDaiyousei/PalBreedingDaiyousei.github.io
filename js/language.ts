// UI Text Keys
export const UI_TEXT_TITLE = "Palworld Breeding Path Finder";
export const UI_TEXT_LANGUAGE = "Language:";
export const UI_TEXT_CURRENT_SELECTIONS = "Current Selections:";
export const UI_TEXT_DESIRED_PAL = "Desired Pal:";
export const UI_TEXT_NONE = "None";
export const UI_TEXT_REQUIRED_PARENTS = "Required Parents:";
export const UI_TEXT_MAX_FORMULA_BUDGET = "Max Formula Budget:";
export const UI_TEXT_MIN_FORMULA_TO_USE = "Min Formula to Use:";
export const UI_TEXT_BREEDING_TREES = "Breeding Trees:";
export const UI_TEXT_SELECT_DESIRED_PAL = "Select a desired pal.";
export const UI_TEXT_SEARCH_FOR_PAL = "Search for a pal...";
export const UI_TEXT_SEARCH_FOR_PARENTS = "Search for parents...";
export const UI_TEXT_NO_VALID_TREE = "No valid breeding tree found within the specified steps.";
export const UI_TEXT_TREE_NUMBER = "Tree #";
export const UI_TEXT_MAX_4_PARENTS = "You can select a maximum of 4 parents.";
export const UI_TEXT_CALCULATING = "Calculating...";
export const UI_TEXT_TOGGLE_THEME = "Toggle Theme";
export const UI_TEXT_SAVE_QUERY = "Save Query";
export const UI_TEXT_PLEASE_SELECT_DESIRED_PAL = "Please select a desired pal first.";
export const UI_TEXT_FAVORITE_QUERIES = "Favorite Queries";

const zhCN: { name: string; texts: { [key: string]: string } } = {
    "name": "简体中文",
    "texts": {

        // UI
        [UI_TEXT_TITLE]: "幻兽帕鲁配种路径查找器",
        [UI_TEXT_LANGUAGE]: "语言：",
        [UI_TEXT_CURRENT_SELECTIONS]: "当前选择：",
        [UI_TEXT_DESIRED_PAL]: "目标帕鲁：",
        [UI_TEXT_NONE]: "无",
        [UI_TEXT_REQUIRED_PARENTS]: "必要帕鲁：",
        [UI_TEXT_MAX_FORMULA_BUDGET]: "最大配种次数：",
        [UI_TEXT_MIN_FORMULA_TO_USE]: "最少配种次数：",
        [UI_TEXT_BREEDING_TREES]: "配种方案：",
        [UI_TEXT_SELECT_DESIRED_PAL]: "请选择目标帕鲁以查看配种方案。",
        [UI_TEXT_SEARCH_FOR_PAL]: "搜索帕鲁...",
        [UI_TEXT_SEARCH_FOR_PARENTS]: "搜索帕鲁...",
        [UI_TEXT_NO_VALID_TREE]: "在指定的步数内没有找到有效的配种方案。",
        [UI_TEXT_TREE_NUMBER]: "方案 #",
        [UI_TEXT_MAX_4_PARENTS]: "最多只能选择4个必要帕鲁。",
        [UI_TEXT_CALCULATING]: "计算中。没有一只帕鲁正在被屠宰...",
        [UI_TEXT_TOGGLE_THEME]: "切换主题",
        [UI_TEXT_SAVE_QUERY]: "保存查询",
        [UI_TEXT_PLEASE_SELECT_DESIRED_PAL]: "请先选择一个目标帕鲁。",
        [UI_TEXT_FAVORITE_QUERIES]: "收藏的查询",

        // Pals
        "Lamball": "棉悠悠",
        "Cattiva": "捣蛋猫",
        "Chikipi": "皮皮鸡",
        "Lifmunk": "翠叶鼠",
        "Foxparks": "火绒狐",
        "Foxparks Cryst": "雪绒狐",
        "Fuack": "冲浪鸭",
        "Fuack Ignis": "热浪鸭",
        "Sparkit": "伏特喵",
        "Tanzee": "新叶猿",
        "Rooby": "燎火鹿",
        "Pengullet": "企丸丸",
        "Pengullet Lux": "闪丸丸",
        "Penking": "企丸王",
        "Penking Lux": "闪丸王",
        "Jolthog": "電棘鼠",
        "Jolthog Cryst": "冰刺鼠",
        "Gumoss": "叶泥泥",
        "Vixy": "玉藻狐",
        "Hoocrates": "啼卡尔",
        "Teafant": "壶小象",
        "Depresso": "瞅什魔",
        "Cremis": "米露菲",
        "Daedream": "寐魔",
        "Rushoar": "草莽猪",
        "Nox": "露娜蒂",
        "Fuddler": "遁地鼠",
        "Killamari": "勾魂鱿",
        "Killamari Primo": "蚀魂鱿",
        "Mau": "喵丝特",
        "Mau Cryst": "冰丝特",
        "Celaray": "鲁米儿",
        "Celaray Lux": "雷米儿",
        "Direhowl": "猎狼",
        "Tocotoco": "炸蛋鸟",
        "Flopie": "波娜兔",
        "Mozzarina": "波霸牛",
        "Bristla": "荊棘魔仙",
        "Gobfin": "鲨小子",
        "Gobfin Ignis": "红小鲨",
        "Hangyu": "吊缚灵",
        "Hangyu Cryst": "冰缚灵",
        "Mossanda": "叶胖达",
        "Mossanda Lux": "雷胖达",
        "Woolipop": "棉花糖",
        "Caprity": "灌木羊",
        "Caprity Noct": "郁木羊",
        "Melpaca": "美露帕",
        "Eikthyrdeer": "紫霞鹿",
        "Eikthyrdeer Terra": "祇岳鹿",
        "Nitewing": "疾风隼",
        "Ribbuny": "姬小兔",
        "Ribbuny Botan": "艾小兔",
        "Incineram": "炎魔羊",
        "Incineram Noct": "暗魔羊",
        "Cinnamoth": "幻悦蝶",
        "Arsox": "炽焰牛",
        "Dumud": "趴趴鲶",
        "Dumud Gild": "梆梆鲶",
        "Cawgnito": "黑鸦隐士",
        "Leezpunk": "朋克蜥",
        "Leezpunk Ignis": "热血蜥",
        "Loupmoon": "月镰魔",
        "Loupmoon Cryst": "霜镰魔",
        "Galeclaw": "天擒鸟",
        "Robinquill": "羽箭射手",
        "Robinquill Terra": "山岳射手",
        "Gorirat": "铁拳猿",
        "Gorirat Terra": "石掌猿",
        "Beegarde": "骑士蜂",
        "Elizabee": "女皇蜂",
        "Grintale": "笑魇猫",
        "Swee": "毛掸儿",
        "Sweepa": "毛老爹",
        "Chillet": "疾旋鼬",
        "Chillet Ignis": "桃旋鼬",
        "Univolt": "雷角马",
        "Foxcicle": "吹雪狐",
        "Pyrin": "火麒麟",
        "Pyrin Noct": "邪麒麟",
        "Reindrix": "严冬鹿",
        "Rayhound": "霹雳犬",
        "Kitsun": "苍焰狼",
        "Kitsun Noct": "幽焰狼",
        "Dazzi": "雷鸣童子",
        "Dazzi Noct": "天阴童子",
        "Lunaris": "秘斯媞雅",
        "Dinossom": "花冠龙",
        "Dinossom Lux": "雷冠龙",
        "Surfent": "滑水蛇",
        "Surfent Terra": "流沙蛇",
        "Maraith": "噬魂兽",
        "Digtoise": "碎岩龟",
        "Tombat": "猫蝠怪",
        "Lovander": "博爱蜥",
        "Flambelle": "融焰娘",
        "Vanwyrm": "烽歌龙",
        "Vanwyrm Cryst": "霜歌龙",
        "Bushi": "浪刃武士",
        "Bushi Noct": "鬼刃武士",
        "Beakon": "迅雷鸟",
        "Ragnahawk": "燧火鸟",
        "Katress": "暗巫猫",
        "Katress Ignis": "炽巫猫",
        "Wixen": "焰巫狐",
        "Wixen Noct": "幽巫狐",
        "Verdash": "踏春兔",
        "Vaelet": "薇莉塔",
        "Sibelyx": "绸笠蛾",
        "Elphidran": "精灵龙",
        "Elphidran Aqua": "水灵龙",
        "Kelpsea": "水灵儿",
        "Kelpsea Ignis": "火灵儿",
        "Azurobe": "碧海龙",
        "Azurobe Cryst": "碧月龙",
        "Cryolinx": "冰棘兽",
        "Cryolinx Terra": "金棘兽",
        "Blazehowl": "狱焰王",
        "Blazehowl Noct": "狱阎王",
        "Relaxaurus": "佩克龙",
        "Relaxaurus Lux": "派克龙",
        "Broncherry": "连理龙",
        "Broncherry Aqua": "海誓龙",
        "Petallia": "花丽娜",
        "Reptyro": "熔岩兽",
        "Reptyro Cryst": "寒霜兽",
        "Kingpaca": "君王美露帕",
        "Kingpaca Cryst": "冰帝美露帕",
        "Mammorest": "森猛犸",
        "Mammorest Cryst": "雪猛犸",
        "Wumpo": "白绒雪怪",
        "Wumpo Botan": "绿苔绒怪",
        "Warsect": "铠格力斯",
        "Warsect Terra": "格鲁力斯",
        "Fenglope": "云海鹿",
        "Fenglope Lux": "雷隐鹿",
        "Felbat": "夜幕魔蝠",
        "Quivern": "天羽龙",
        "Quivern Botan": "翠羽龙",
        "Blazamut": "焰煌",
        "Blazamut Ryu": "殁殃",
        "Helzephyr": "雷冥鸟",
        "Helzephyr Lux": "雷鸣鸟",
        "Astegon": "魔渊龙",
        "Menasting": "冥铠蝎",
        "Menasting Terra": "金铠蝎",
        "Anubis": "阿努比斯",
        "Jormuntide": "覆海龙",
        "Jormuntide Ignis": "腾炎龙",
        "Suzaku": "朱雀",
        "Suzaku Aqua": "清雀",
        "Grizzbolt": "暴电熊",
        "Lyleen": "百合女王",
        "Lyleen Noct": "黑月女王",
        "Faleris": "荷鲁斯",
        "Faleris Aqua": "伊西斯",
        "Orserk": "波鲁杰克斯",
        "Shadowbeak": "异构格里芬",
        "Paladius": "圣光骑士",
        "Necromus": "混沌骑士",
        "Frostallion": "唤冬兽",
        "Frostallion Noct": "唤夜兽",
        "Jetragon": "空涡龙",
        "Bellanoir": "贝菈诺娃",
        "Bellanoir Libero": "贝菈露洁",
        "Selyne": "辉月伊",
        "Croajiro": "武道蛙",
        "Croajiro Noct": "极道蛙",
        "Lullu": "春彩娘",
        "Shroomer": "菇咚",
        "Shroomer Noct": "菇波",
        "Kikit": "球犰",
        "Sootseer": "恐炬灵",
        "Prixter": "蛊刺妖",
        "Knocklem": "泰锋",
        "Yakumo": "八云犬",
        "Dogen": "汪宗师",
        "Dazemu": "战冠雀",
        "Mimog": "旺财",
        "Xenovader": "杰诺贝达",
        "Xenogard": "杰诺路达",
        "Xenolord": "杰诺多兰",
        "Nitemary": "魅爱莉",
        "Starryon": "夜冥驹",
        "Silvegis": "艾基鲁迦",
        "Smokie": "墨丸",
        "Celesdir": "净世鹿",
        "Omascul": "面惧",
        "Splatterina": "幽恋娜",
        "Tarantriss": "桃蛛娘",
        "Azurmane": "驭雷马",
        "Bastigor": "霜牙王",
        "Prunelia": "梅莉姆",
        "Nyafia": "妮瞅莎",
        "Gildane": "金驰兽",
        "Herbil": "达鼠泥",
        "Icelyn": "冰姬灵",
        "Frostplume": "白真雪雀",
        "Palumba": "咕咕桑葩",
        "Braloha": "梁叶龙",
        "Munchill": "肚肚鳄",
        "Polapup": "香草豹冰",
        "Turtacle": "盔盔仔",
        "Turtacle Terra": "金盔仔",
        "Jellroy": "海月灵",
        "Jelliette": "海月仙",
        "Gloopie": "墨沫姬",
        "Finsider": "布偶鲨",
        "Finsider Ignis": "粉粉布偶鲨",
        "Ghangler": "冥灯鱼",
        "Ghangler Ignis": "炙灯鱼",
        "Whalaska": "凉晶鲸",
        "Whalaska Ignis": "桃晶鲸",
        "Neptilius": "海皇鲸",
        "Green Slime": "绿史莱姆",
        "Blue Slime": "蓝史莱姆",
        "Red Slime": "红史莱姆",
        "Purple Slime": "紫史莱姆",
        "Illuminant Slime": "夜明史莱姆",
        "Rainbow Slime": "彩虹史莱姆",
        "Enchanted Sword": "附魔剑",
        "Cave Bat": "洞穴蝙蝠",
        "Illuminant Bat": "夜明蝙蝠",
        "Eye of Cthulhu": "克苏鲁之眼",
        "Demon Eye": "恶魔眼"
    }
}

export type LanguageCode = 'en-US' | 'zh-CN';

export function getLanguageText(langCode: LanguageCode, textKey: string) : string {
    if (langCode === "en-US") {
        return textKey;
    }

    const lang = zhCN;
    return lang.texts[textKey] || textKey;
}