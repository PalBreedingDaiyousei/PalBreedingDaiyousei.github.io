import { palRecords, PalBreedingInfo, PalName } from './pals.js';

class BreedingFormula {
    readonly child: PalName;
    readonly lhs: PalName;
    readonly rhs: PalName;

    constructor(child: PalName, lhs: PalName, rhs: PalName) {
    ...
    hasParent(parent: PalName) {
        return (parent === this.lhs || parent === this.rhs);
    }

    hasAnyParent(parents: Set<PalName>) {
        return parents.has(this.lhs) || parents.has(this.rhs);
    }

    hasBothParent(parent1: PalName, parent2: PalName) {
        const [p1, p2] = parent1 <= parent2 ? [parent1, parent2] : [parent2, parent1];
        return (p1 === this.lhs && p2 === this.rhs);
    }

    private static allFormulas = function () {
        const pals = Object.entries(palRecords) as [PalName, PalBreedingInfo][];
        const formulas: Map<PalName, BreedingFormula[]> = new Map(pals.map(([name, _]) => [name, []]));

        // Exception formulas are predefined.
        const exceptionSet: Set<string> = new Set();
        for (const [palName, breedingInfo] of pals) {
            const addExceptionFormula = function(lhs: PalName, rhs: PalName) {
            ...
            if (breedingInfo.exception !== null) {
                if (typeof breedingInfo.exception[0] === "string") {
                    const [lhs, rhs] = breedingInfo.exception as [PalName, PalName];
                    addExceptionFormula(lhs, rhs);
                }
                else {
                    for (const [lhs, rhs] of breedingInfo.exception as [PalName, PalName][]) {
                        addExceptionFormula(lhs, rhs);
                    }
                }
            }
    ...
    static findFormula(child: PalName) : ReadonlyArray<BreedingFormula> {
        return BreedingFormula.allFormulas.get(child) || [];
    }
}

export class BreedingTreeNode {
    constructor(readonly formula: BreedingFormula, readonly left: BreedingTreeNode | null = null, readonly right: BreedingTreeNode | null = null) {
    }

    cloneReplacingLeft(newLeft: BreedingTreeNode | null) : BreedingTreeNode {
        return new BreedingTreeNode(this.formula, newLeft, this.right);
    }

    cloneReplacingRight(newRight: BreedingTreeNode | null) : BreedingTreeNode {
        return new BreedingTreeNode(this.formula, this.left, newRight);
    }

    dump() {
        function dumpAux(self: BreedingTreeNode, indent: string) {
...
class BreedingSearchContext {
    ...
    expandBreedingTree(rootFormula: BreedingFormula, requiredParents: ReadonlySet<PalName>, excludingPals: ReadonlySet<PalName>, nFormula: number) : BreedingTreeNode[] {
        ...
        const results: BreedingTreeNode[] = [];
        for (let nLhsFormula = 0; nLhsFormula < nFormula; nLhsFormula++) {
            const nRhsFormula = nFormula - 1 - nLhsFormula;
            ...
            for (const [lhsSharedRequiredParents, rhsSharedRequiredParents] of this.splitSharedRequiredParents(sharedRequiredParents)) {
                const lhsSubtrees: (BreedingTreeNode | null)[] = [];
                if (nLhsFormula >= 1) {
                ...
                const rhsSubtrees: (BreedingTreeNode | null)[] = [];
                if (nRhsFormula >= 1) {
    ...
    findBreedingTree(child: PalName, requiredParents: ReadonlySet<PalName>, maxFormulaBudget: number, minFormulaBudget: number) : BreedingTreeNode[] {
        ...
        for (let usedFormulaNum = Math.max(minFormulaBudget, requiredParents.size - 1); usedFormulaNum <= maxFormulaBudget; usedFormulaNum++) {
            ...
            const result = childFormulas.flatMap(formula => 
                this.expandBreedingTree(formula, requiredParents, new Set([child]), usedFormulaNum)
            );
            if (result.length > 0) {
